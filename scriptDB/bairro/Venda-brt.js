...
setTimeout(()=>{
    ...
    for (let index = 0; index <= 9; index++) {
            ...
            querriesVenda.push(`INSERT INTO Venda VALUES("${VendaID}",${tarifas[number]})`)    
    }
    dataBrt.forEach(item =>{
        const bairro = item[1].normalize('NFD').replace(/[\u0300-\u036f]/g, "").toUpperCase();
        const BairroFiltrado = data.filter(BairroData => {
            const NomeBairro = BairroData.Nome_Bairro.normalize('NFD').replace(/[\u0300-\u036f]/g, "").toUpperCase().split(",")[0];
            return NomeBairro ==bairro;
        })
        const EstacaoId = crypto.randomUUID()
        querriesEstacao.push(`INSERT INTO Estacao VALUES("${EstacaoId}", true, "${item[0]}","${BairroFiltrado[0].ID_Bairro}")`);
        for (let index = 0; index < vendaIDS.length; index++) {
            const vendaId = vendaIDS[index];
            const ano = 2012+index
            const indexItem = 2+index
            querriesVendaEstacao.push(`INSERT INTO VendaEstacao VALUES(${ano},${item[indexItem] == "-"? null:parseInt(item[indexItem].replace(/\s/g, ''))},"${vendaId}", "${EstacaoId}")`)
